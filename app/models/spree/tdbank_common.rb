  ...
  module TdbankCommon
    ...
    included do
      ...
      def merchant_account
        ENV['TDBANK_MERCHANT_ACCOUNT'] || preferred_merchant_account
      end
      ...
      def provider
        ::Tdbank.configuration.api_username = (ENV['TDBANK_API_USERNAME'] || preferred_api_username)
        ::Tdbank.configuration.api_password = (ENV['TDBANK_API_PASSWORD'] || preferred_api_password)
        ::Tdbank.configuration.default_api_params[:merchant_account] = merchant_account
        ...
        def set_up_contract(source, card, user, shopper_ip)
          ...
          if response.success?
            ...
            if last_digits.blank? && payment_profiles_supported?
              ...
              raise Tdbank::MissingCardSummaryError, note
            end
        ...
        def create_profile_on_card(payment, card)
          unless payment.source.gateway_customer_profile_id.present?
            ...
            if response.success?
              ...
              raise Tdbank::Enrolled3DError.new(response, payment.payment_method)
            else
              logger.error(Spree.t(:gateway_error))
              logger.error("  #{response.to_yaml}")
              raise Core::GatewayError.new(response.fault_message || response.refusal_reason)
