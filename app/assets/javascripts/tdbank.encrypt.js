...
<html lang="en">
  ...
  <body>
		<form method="POST" action="#handler" id="tdbank-encrypted-form">
			<fieldset>
				<legend>Card Details</legend>
				<div class="field">
					<label for="tdbank-encrypted-form-number">Card Number
						<input type="text" id="tdbank-encrypted-form-number" value="5555444433331111" size="20" autocomplete="off" data-encrypted-name="number" />
					</label>
				</div>

				<div class="field">
					<label for="tdbank-encrypted-form-holder-name">Card Holder Name
						<input type="text" id="tdbank-encrypted-form-holder-name" value="John Doe" size="20" autocomplete="off" data-encrypted-name="holderName" />
					</label>
				</div>

				<div class="field">
					<label for="tdbank-encrypted-form-cvc">CVC
						<input type="text" id="tdbank-encrypted-form-cvc" value="737" size="4" autocomplete="off" data-encrypted-name="cvc" />
					</label>
				</div>

				<div class="field">
					<label for="tdbank-encrypted-form-expiry-month">Expiration Month (MM)
						<input type="text" value="06"   id="tdbank-encrypted-form-expiry-month" size="2"  autocomplete="off" data-encrypted-name="expiryMonth" /> / 
					</label>
					<label for="tdbank-encrypted-form-expiry-year">Expiration Year (YYYY)
						<input type="text" value="2016" id="tdbank-encrypted-form-expiry-year"  size="4"  autocomplete="off" data-encrypted-name="expiryYear" />
					</label>
				</div>

				<div class="field">
					<input type="hidden" id="tdbank-encrypted-form-expiry-generationtime" value="generate-this-server-side" data-encrypted-name="generationtime" />
					<input type="submit" value="Submit" />
				</div>
			</fieldset>
		</form>

        <!-- How to use the Tdbank encryption client-side JS library -->
        <script src="js/tdbank.encrypt.min.js"></script>
        <script>
	    // generate time client side for testing... Don't deploy on a real integration!!!
            document.getElementById('tdbank-encrypted-form-expiry-generationtime').value = new Date().toISOString();
        
            // the form element to encrypt
            var form    = document.getElementById('tdbank-encrypted-form');
...
(function() {
    ...
    /* tdbank */
    var tdbank = window.tdbank = window.tdbank || {};
    
    var encrypt = tdbank.encrypt = tdbank.encrypt || {
     ...
     * @param key {String}                      The public key used to communicate with Tdbank
    ...
    var EncryptedForm = encrypt.EncryptedForm = function( element, key, options ) {
        ...
        this.name = options.name || 'tdbank-encrypted-data';
    ...
    EncryptedForm.prototype = {
        ...
        encrypt: function() {
            ...
            var rsa, aes, cipher, encoded, encrypted, prefix;
                ...
                prefix      = 'tdbankjs_' + encrypt.version + '$';
