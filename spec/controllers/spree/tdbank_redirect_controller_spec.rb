...
module Spree
  describe TdbankRedirectController do
    let(:order) { create(:order_with_line_items, state: "payment") }

    context "Tdbank HPP Gateway" do
      def params
      ...
      let(:payment_method) { Gateway::TdbankHPP.create(name: "Tdbank") }
    ...
    context "Tdbank 3-D redirect" do
      let(:env) do
        {
      ...
      context "stubbing Tdbank API" do
        let(:params) do
          { MD: "Sooo", PaRes: "Wat" }
        end

        let!(:gateway) { Gateway::TdbankPaymentEncrypted.create!(name: "Tdbank") }

        before do
          expect(controller).to receive(:current_order).and_return order

          expect(Gateway::TdbankPaymentEncrypted).to receive(:find).and_return gateway
        ...
        it "redirects user if no recurring detail is returned" do
          spree_get :authorise3d, params, { tdbank_gateway_name: gateway.class.name, tdbank_gateway_id: gateway.id }
      ...
      context "reaching Tdbank API", external: true do
        let(:params) do
          { MD: test_credentials["controller_md"], PaRes: test_credentials["controller_pa_response"] }
        end

        let!(:gateway) do
          Gateway::TdbankPaymentEncrypted.create!(
            name: "Tdbank",
